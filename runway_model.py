...
@runway.setup(options={"model_size" : category(choices=['xlnet-base-cased', 'xlnet-large-cased'], default='xlnet-base-cased')})
def setup(opts):
    ...
    tokenizer = tokenizer_class.from_pretrained(model_size)
    model = model_class.from_pretrained(model_size)
    model.to(device)
    model.eval()
...
command_inputs = {
    ...
    "num_samples" : number(default=1, min=1, step=1, description="Number of samples per prediction"),
